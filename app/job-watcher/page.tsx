...
import { Search, Plus, RefreshCw, ExternalLink, ChevronDown, ChevronRight, FileText, Trash2 } from 'lucide-react';
...
export default function JobWatcher() {
  ...
  const refreshSource = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        addToast(data.error || 'Scan failed — unknown error.', 'error');
        return;
      }
      ...
      addToast('Network error — could not reach the scan API.', 'error');
    } finally {
      setRefreshing(null);
    }
  };
  ...
  const deleteJob = (sourceId: string, jobId: string) => {
    const updated = sources.map(s => {
      if (s.id === sourceId) {
        return { ...s, jobs: s.jobs.filter((j: any) => j.id !== jobId) };
      }
      return s;
    });
    saveToStorage(updated);
    addToast('Job entry removed.', 'success');
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden text-black">
      {/* Fixed Header & Add Form Area */}
      <div className="flex-shrink-0 p-4 pt-16 lg:pt-8 lg:p-12 pb-0 w-full max-w-5xl mx-auto">
          ...
          <form onSubmit={addSource} className="flex flex-col sm:flex-row gap-6 items-end">
            ...
            <div className="flex-1 w-full">
              ...
              <input 
                ...
                className="w-full px-4 py-3 border-4 border-black focus:outline-none focus:bg-[#e8fc3b]/10 font-mono text-sm"
                placeholder="e.g. boards.greenhouse.io/company"
              />
            </div>
            <div className="flex-1 w-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-12 pt-0 w-full max-w-5xl mx-auto">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 bg-[#f4f4f0] border-b-4 border-black">
                <div className="flex items-center cursor-pointer flex-1 mb-4 sm:mb-0" onClick={() => toggleExpand(source.id)}>
                  ...
                  <div>
                    ...
                    <a href={source.url} target="_blank" rel="noreferrer" className="text-xs font-mono text-black/60 hover:text-blue-600 hover:underline flex items-center mt-1 max-w-[200px] sm:max-w-md" onClick={e => e.stopPropagation()}>
                      <span className="truncate">{source.url}</span> <ExternalLink className="w-3 h-3 ml-1 flex-shrink-0" />
                    </a>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  {confirmDeleteId === source.id ? (
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => performDelete(source.id)}
                        className="px-3 py-1 bg-red-500 text-white border-2 border-black font-bold uppercase text-[10px] tracking-widest hover:bg-black transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none"
                      >
                        Confirm?
                      </button>
                      <button
                        onClick={() => setConfirmDeleteId(null)}
                        className="px-3 py-1 bg-white border-2 border-black font-bold uppercase text-[10px] tracking-widest hover:bg-gray-100 transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <>
                      <span className="text-sm font-mono font-bold bg-black text-white px-3 py-1 uppercase tracking-widest whitespace-nowrap">
                  ...
                  {source.jobs?.length === 0 ? (
                    ...
                    source.jobs?.map((job: any) => (
                      <div key={job.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between group hover:bg-[#f4f4f0] transition-colors">
                        ...
                        <button
                          onClick={() => deleteJob(source.id, job.id)}
                          className="opacity-0 group-hover:opacity-100 p-2 bg-white border-2 border-black hover:bg-red-500 hover:text-white transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none"
                        >
